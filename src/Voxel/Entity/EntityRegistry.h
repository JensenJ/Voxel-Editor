...
class EntityRegistry {
  ...
  public:
    ...
    template <typename T> bool RegistryHasComponentType() {
        ...
        for (std::map<Component*, std::vector<Component*>>::iterator iter =
                 componentRegistry.begin();
             iter != componentRegistry.end(); ++iter) {
            // If this component is of type T
            Component& c = *iter->first;
    ...
    Component* AddComponentOfTypeToRegistry(unsigned int entity, Args... args) {
        ...
        // This type of component does not exist in the registry if we have not added it yet, so
        // create a new vector of this type and add to registry
