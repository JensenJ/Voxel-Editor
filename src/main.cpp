...
int main() {
    ...
    camera->AddComponent<CameraComponent>(glm::vec3(0.0f, 0.0f, 0.0f), YAW, PITCH, MOVEMENTSPEED,
                                          SENSITIVITY, ZOOM);
    ...
    for (int x = 0; x < 3; x++) {
        for (int y = 0; y < 2; y++) {
            for (int z = 0; z < 10; z++) {
                Entity* entity =
                    entityRegistry.CreateEntity("Cube (" + std::to_string(x) + ", " +
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 projection =
            glm::perspective(glm::radians(camComponent->GetZoom()),
...
#pragma warning(disable : 4312)
        ImGui::Image(ImTextureID(sceneBuffer->GetFrameTexture()), ImGui::GetContentRegionMax(),
